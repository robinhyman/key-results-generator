...
- The PR-report gate reads evidence within the section that claims it, not anywhere in the body, as of issue #31. Both bypasses found by the 2026-08-16 external audit — an empty body, and four empty headings plus a stray URL and delegation sentence — are pinned as regression tests.
...
- `increment-check`'s report gate is covered by `test/increment-check.test.js` (43 PR-body fixtures) as of issue #31. Its git-range, secrets, budget, and freshness checks are still verified only by manual runs.
